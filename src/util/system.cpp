...
const char * const BITCOIN_CONF_FILENAME = "wednesdaydudecoin.conf";
...
#else
    const char* pszModule = "wednesdaydudecoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Wednesdaydudecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Wednesdaydudecoin";
#else
    // Unix
    return pathRet / ".wednesdaydudecoin";
#endif
#endif
}
