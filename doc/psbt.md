# PSBT Howto for Wednesdaydudecoin Core

Since Wednesdaydudecoin Core 0.17, an RPC interface exists for Partially Signed Wednesdaydudecoin
...
PSBT is an interchange format for Wednesdaydudecoin transactions that are not fully signed
...
## PSBT in Wednesdaydudecoin Core
...
#### Multisig with multiple Wednesdaydudecoin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Wednesdaydudecoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `wednesdaydudecoin-cli` is used.
